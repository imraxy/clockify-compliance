...
type ComplianceRow = ComplianceMonth['rows'][number];
type ComplianceDayCell = ComplianceRow['days'][string];
type OverrideRow = {
  user_email: string;
  date: string;
  status: string;
...
function DashboardPage({ token, addNotification }: { token: string; addNotification: (t: 'success' | 'error' | 'info', m: string) => void }) {
  ...
  const [selectedCell, setSelectedCell] = useState<{ user: ComplianceRow; day: number } | null>(null);
  ...
  }, [token, year, month, addNotification]);
  ...
  const stats = useMemo(() => {
    ...
    data.rows.forEach(row => {
      Object.values(row.days).forEach((cell: ComplianceDayCell) => {
...
function ReportsPage({ token, addNotification }: { token: string; addNotification: (t: 'success' | 'error' | 'info', m: string) => void }) {
  ...
  const anomalyList = useMemo(() => {
    ...
    data.rows.forEach(row => {
      Object.entries(row.days).forEach(([day, cell]) => {
...
function ImportPage({ token, addNotification }: { token: string; addNotification: (t: 'success' | 'error' | 'info', m: string) => void }) {
  ...
  async function uploadAttendance() {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        addNotification('success', `Imported ${data.imported ?? data.rows ?? 0} attendance records`);
  ...
  async function uploadTimeEntries() {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        addNotification('success', `Imported ${data.imported ?? data.rows ?? 0} time entries`);
    ...
    <div className="page">
      ...
      <div className="sync-status-card card">
        <div className="sync-info">
          <h2>Manual Sync</h2>
          <p className="sync-status">Run Clockify sync manually for a recent date range.</p>
      ...
      <div className="sync-help card">
        ...
        <ul className="help-list">
          ...
          <li>Manual sync keeps data fresh until a scheduler is configured</li>
...
function JiraPage() {
  return (
    <div className="page">
      ...
      <div className="card">
        <h3 className="card-title">Phase 2 integration</h3>
        <p>
          Jira variance reporting is planned for phase 2. The backend currently exposes a helper
          that compares prepared estimate and actual hour maps; live Jira ingestion is not wired yet.
        </p>
        <p className="hint">Next step: add Jira issue import, then connect it to this report.</p>
      </div>
    </div>
  );
}
    ...
    <div className="page">
      ...
      <div className="settings-grid">
        ...
        <div className="settings-card card">
          ...
          <p>API Docs: <a href={`${getApiUrl()}/docs`} target="_blank" rel="noopener noreferrer" className="link">OpenAPI Documentation</a></p>
          <p>Health: <a href={`${getApiUrl()}/health`} target="_blank" rel="noopener noreferrer" className="link">System Status</a></p>
        </div>

        <div className="settings-card card">
          <h2>🔄 Sync</h2>
          <p>Clockify sync is currently run manually from the Sync page.</p>
          <p>Configure a scheduler before advertising automatic daily runs.</p>
        </div>
      </div>
    </div>
  );
